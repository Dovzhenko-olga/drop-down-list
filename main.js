...
class DropDownList {
  ...
  _onElementInput({target}) {
    ...
    this.createListMarcup(this.data.filter(elem => elem.toLowerCase().indexOf(target.value.toLowerCase()) !== -1));
  ...
  createListMarcup(data) {
    this.listElement = document.createElement(`ul`);
    this.listElement.className = `drop-list`;
    this.listElement.innerHTML = data.map(elem => `<li class="drop-item">${elem}</li>`).join(``);
...
new DropDownList({ element: input, data });